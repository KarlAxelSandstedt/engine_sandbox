...
#define PlatformFreeAligned(ptr)	free(ptr)
#else
#error
#endif
...
#include <malloc.h>
#define PlatformFreeAligned(ptr)	_aligned_free(ptr)
...
//	ds_AssertString(((u64) allocator->block & (g_arch_config->cacheline-1)) == 0, "allocator block array should be cacheline aligned");
//	if (!allocator->block)
//	{
...
//{
//	ds_StaticAssert(sizeof(struct pool_external_slot) == 4, "Expect size of pool_external_slot is 4");
//
//	*external_buf = NULL;
