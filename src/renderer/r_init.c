...
void r_init(struct arena *mem_persistent, const u64 ns_tick, const u64 frame_size, const u64 core_unit_count)
{
	...
	g_r_core->program[PROGRAM_PROXY3D].buffer_shared_layout_setter = NULL;
	g_r_core->program[PROGRAM_PROXY3D].buffer_local_layout_setter = r_proxy3d_buffer_layout_setter;
	...
	//vec2u32 size;
	//window_size(size, sys_win->native);
