...
{
	kas_static_assert(BIT_VEC_BLOCK_SIZE == 64, "must be a power of two");
}

struct bit_vec bit_vec_alloc(struct arena *mem, const u64 bit_count, const u64 clear_bit, const u32 growable)
{
	kas_assert_string(bit_count >= 1 && clear_bit <= 1, "invalid bit_vec_alloc bit count or clear bit value");
...
{
	kas_assert(bvec->bit_count < bit_count);
	kas_assert(bvec->growable);
...
{
	kas_assert(bit < bvec->bit_count && bit_value <= 1);
